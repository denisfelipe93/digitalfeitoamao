...
<template>
  <section class="relative py-20 lg:py-28 bg-[var(--paper)]" aria-labelledby="testimonials-title"
    ...
    <div class="container">
      ...
      <header class="text-center max-w-4xl mx-auto mb-12 lg:mb-16">
        <h2 id="testimonials-title"
            class="font-sans font-extrabold text-[var(--ink)] tracking-tight
                   text-[clamp(30px,4.6vw,52px)]">
          {{ title }}
        </h2>
        ...
        <div class="mt-6 flex justify-center">
          <span class="ribbon">
            <svg class="h-[18px] w-[18px] opacity-70" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M12 2c.6 2.9 2.5 4.7 5.4 5.4C14.5 8 12.7 9.9 12 12c-.6-2.1-2.5-4-5.4-4.6C9.5 6.7 11.4 4.9 12 2Zm7 10c.4 1.7 1.5 2.8 3.1 3.1-1.6.3-2.7 1.4-3.1 3.1-.3-1.6-1.4-2.8-3.1-3.1 1.7-.3 2.8-1.4 3.1-3.1ZM2 14c.4 2 1.9 3.5 3.9 3.9C3.9 18.3 2.4 19.8 2 22c-.3-2.2-1.8-3.7-4-4.1C0.2 17.6 1.7 16.1 2 14Z"/>
            </svg>
      ...
      <div class="mt-16 lg:mt-32 grid md:grid-cols-2 lg:grid-cols-3 gap-y-14 gap-x-12 lg:gap-x-16">
        <figure v-for="(t, i) in items" :key="t.id"
                class="group card relative text-center pt-24 pb-12 px-8 md:px-10
                       shadow-[0_20px_50px_rgba(0,0,0,.08)]
                       transition-transform motion-safe:hover:-translate-y-1
                       focus-within:-translate-y-1"
                :class="blobClass(i)">
          <!-- avatar -->
          <div class="absolute -top-12 left-1/2 -translate-x-1/2 h-24 w-24 rounded-full ring-[6px] ring-white overflow-hidden shadow-md">
          ...
          <svg class="mx-auto mb-4 h-6 w-6 text-black/25" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M7.17 6.03C4.9 7.3 3.5 9.6 3.5 12.14c0 3 2.12 5.36 5.24 5.36 2.62 0 4.76-2.14 4.76-4.76 0-2.62-2.14-4.76-4.76-4.76-.28 0-.56.02-.81.05.37-1.1 1.11-2.01 2.2-2.68L7.17 6.03Zm9 0c-2.27 1.27-3.67 3.57-3.67 6.11 0 3 2.14 5.36 5.26 5.36 2.62 0 4.76-2.14 4.76-4.76 0-2.62-2.14-4.76-4.76-4.76-.28 0-.56.02-.81.05.37-1.1 1.11-2.01 2.2-2.68l-2.98-.32Z"/>
          </svg>
...
const IconCup = (p:any) => h('svg', { ...p, viewBox:'0 0 24 24', fill:'currentColor','aria-hidden':'true' }, [
...
const IconChip = (p:any) => h('svg', { ...p, viewBox:'0 0 24 24', fill:'currentColor','aria-hidden':'true' }, [
...
const IconPalette = (p:any) => h('svg', { ...p, viewBox:'0 0 24 24', fill:'currentColor','aria-hidden':'true' }, [
...
const items = [
  ...
  },
  {
    ...
    body:'O conteúdo e os visuais que criaram para nossas redes elevaram a marca. Profissionalismo e criatividade em cada frame.',
  },
  {
    id:'3',
    name:'Sofia Costa',
...
.ribbon::before,
.ribbon::after{
  content:""; position:absolute; top:50%; width:22vw; max-width:280px; height:1px;
...
@media (max-width: 768px){
  .ribbon::before, .ribbon::after{ width:18vw; }
}
